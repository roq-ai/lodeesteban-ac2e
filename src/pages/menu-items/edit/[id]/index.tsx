...
import { useRoqClient, useMenuItemFindFirst } from 'lib/roq';
...
function MenuItemEditPage() {
  ...
  const { data, error, isLoading, mutate } = useMenuItemFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: MenuItemInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.menu_item.update({
        data: values as RoqTypes.menu_item,
        where: {
          id,
        },
      });
